 ...
 */
abstract class ST_Admin_Settings_Page implements ST_Admin_Settings_Page_Interface {
	...
	public function render_fields() {
		...
		settings_fields( 'st_settings_group_' . $this->id );
		...
		foreach ( $settings as $key => $setting ) {
			...
			$setting['id'] = ( $setting['id'] ) ? ST_Admin_Settings::OPTION_NAME_PREFIX . $setting['id'] : '';
			...
			switch ( $setting['type'] ) {
				...
				case 'title' :
					...
					$tag = ( 0 === $key
					         && count( apply_filters( 'st_settings_tabs', array() ) ) < 2
					) ? 'h1' : 'h2';
					if ( $setting['title'] ) {
				...
				case 'password' :
					...
					$value = ST_Admin_settings::get_option( $setting['id'], $setting['default'] );
					...
					<td class="st-field-input st-field-input-checkbox">
						...
						foreach ( $checkboxes as $checkbox ):
							...
							if ( 'checkboxgroup' == $setting['type'] ) {
								$checkbox['id'] = ST_Admin_Settings::OPTION_NAME_PREFIX . $checkbox['id'];
							}

							$value = ST_Admin_settings::get_option( $checkbox['id'], $checkbox['default'] );
